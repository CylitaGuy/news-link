...
and Cosine Similarity (see Data Viz notebook)
...
#Loading in large space english language model
...
for index, row in news_validation.iterrows():
    ...
    #Creating GloVe embeddings for the corpus
    embeds = SIM.GloveEmbeds (corpus = corpus)

    #Computing similarity scores for given dataframe
    scores = SIM.GloVeCosine(querydoc = teststory['cleaned_text'], corpus_embeddings = embeds)
...
#Turn into dataframe and export as csv
val_data = pd.DataFrame(output_validation)
val_data.to_csv("validation_metrics_GloveCosine.csv")
